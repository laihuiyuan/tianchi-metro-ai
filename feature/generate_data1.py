...
def base_features(df_,sample_=None,flag=True):
    ...
    # count,sum
    result = df.groupby(['stationID', 'week', 'weekend', 'day', 'hour', 'minute','payType']).status.agg(
    ...
    # count = temp.groupby(['stationID','hour', 'minute'], as_index=False)['inNums_dID'].agg({
    #     'inDID_hm_max': 'max', 'inDID_hm_min': 'min', 'inDID_hm_mean': 'mean'})
    # result = result.merge(count, on=['stationID','hour', 'minute'], how='left')
    #
    # count = temp.groupby(['stationID','hour', 'minute'], as_index=False)['outNums_dID'].agg({
    #     'outDID_hm_max': 'max', 'outDID_hm_min': 'min', 'outDID_hm_mean': 'mean'})
    # result = result.merge(count, on=['stationID', 'hour', 'minute'], how='left')
...
test=pd.DataFrame()
for i in range(4):
    df_te['payType']=df_te['payType']+1
    data=pd.concat([data,df_te],axis=0)
...
data = data[['stationID','week', 'weekend', 'day', 'hour', 'minute', 'payType','inNums', 'outNums']]

data = data.merge(tmp, on=['stationID','day', 'hour', 'minute','payType'], how='left')
data.fillna(0, inplace=True)
print(data.shape)

# data = data[data.day!=1]
data.to_csv('data_payType.csv',index=False)
